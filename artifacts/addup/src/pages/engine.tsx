...
function EngineLoader({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    LOADER_LINES.forEach((l, i) => {
      ts.push(setTimeout(() => setVisible(p => [...p, i]), l.ms));
    });
...
export default function Engine() {
  ...
  function generatePDF() {
    ...
    doc.setFontSize(8.5);
    MATCHES.forEach((m) => {
      const ql = m.quality === "perfect" ? "Perfect" : m.quality === "close" ? "Close" : "Amount";
      cx = col;
            ...
            >
              ...
              {stepId === "upload" && (
                <section className="max-w-2xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                  <StepLabel n={1} />
                  <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900 mb-2">Upload your two files</h1>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    {[
                      { label: "Bank Statement",  file: "bank.csv",   rows: 19, size: "4.2 KB", note: "FNB Business · April 2026",  tip: "Export this from your online banking portal."   },
                      { label: "General Ledger",  file: "ledger.csv", rows: 11, size: "2.8 KB", note: "Xero export · April 2026",    tip: "Export this from Xero, QuickBooks, or Sage."   },
                    ].map((f) => (
                  ...
                  <Callout>Both files are pre-loaded with sample April 2026 data so you can see the full workflow right now.</Callout>
                </section>
              )}
              ...
              {stepId === "read" && (
                <section className="max-w-2xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                  <StepLabel n={2} />
                  <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900 mb-2">Files read successfully</h1>
                  ...
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-3">Date formats recognized automatically</p>
              ...
              {stepId === "match" && (
                <section className="max-w-2xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                  <StepLabel n={3} />
                  <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900 mb-2">Comparing bank vs. ledger</h1>
                        ...
                        <div key={m.bank.id} className="p-4">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              { label: "Ledger", date: m.ledger.date, desc: m.ledger.desc },
                            ].map(({ label, date, desc }) => (
                    ...
                    {(filter === "all" || filter === "flagged") && MISSING.map((m) => (
                      <div key={m.id} className="p-4 bg-amber-50/40">
                        <div className="flex items-start justify-between gap-3 mb-1">
                          <div className="min-w-0">
                            <span className="text-[10px] font-semibold px-1.5 py-0.5 border bg-amber-50 text-amber-700 border-amber-200 inline-block mb-1.5">Not in ledger</span>
              ...
              {stepId === "review" && (
                <section className="max-w-2xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                  ...
                  <div className="flex items-baseline justify-between gap-4 mb-2">
                    <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900">{ISSUES.length} entries need your attention</h1>
                    <span className="text-xs font-medium text-gray-400 shrink-0">{resolved.size} / {ISSUES.length} done</span>
                  </div>
                  <p className="text-gray-500 text-sm sm:text-[15px] mb-10 leading-relaxed">
                  ...
                  <div className="space-y-3">
                    {ISSUES.map((iss) => {
                      ...
                      return (
                        <div key={iss.id}
                          className={`border p-4 sm:p-5 transition-all duration-200 ${done ? "border-gray-100 opacity-40" : "border-gray-100 bg-white"}`}
                        >
                          <div className="flex items-start justify-between gap-3 mb-3">
              ...
              {stepId === "report" && (
                <section className="max-w-2xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                  ...
                  >
                    ...
                    <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900 mb-3">April 2026 is reconciled</h1>
